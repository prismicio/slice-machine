...
async function publish(options?: Options): Promise<void> {
  ...
  if (packageNameToVersionOrReleaseType !== undefined) {
    ...
    // 1. Browse the version history to find the highest published prerelease
    //    number that matches the current branch / prerelease identifier.
    // 2. Then:
    //    - If we find a matching prerelease number, increment it and keep the
    //      main part of the SemVer string (i.e., the `<major>.<minor>.<patch>`
    //      part) unchanged.
    //    - If we don't find any, initialize the prerelease number to `1` and
    //      increment the patch version number instead.
