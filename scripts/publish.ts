...
async function publish(options?: Options): Promise<void> {
  ...
  const args = mri<{ "dry-run": boolean }>(process.argv.slice(2), {
    alias: { n: "dry-run" },
    boolean: ["dry-run"],
  ...
  packageNameToReleaseVersion.forEach((packageReleaseVersion, packageName) => {
    const packageVersionHistory = packageNameToVersionHistory.get(packageName);
  ...
  for (const nonPrivateWorkspaceName of nonPrivateWorkspaceNames) {
    const nonPrivateWorkspaceReleaseVersion =
      packageNameToReleaseVersion.get(nonPrivateWorkspaceName) ?? "decline";
    await $`yarn workspace ${nonPrivateWorkspaceName} version ${nonPrivateWorkspaceReleaseVersion} --deferred`;
  }
  ...
  const foreachInclude = [...packageNameToReleaseVersion.keys()].join(",");
  let npmTag: string;
  if (mode === "stable") {
    npmTag = "latest";
  } else {
    npmTag = isOnDefaultBranch
      ? maturityLevel
      : `${maturityLevel}+${sanitizedCurrentBranchName}`;
  }
  await $`yarn workspaces foreach --all --include {${foreachInclude}} --no-private --parallel --topological-dev npm publish --access public --tag ${npmTag}`;
