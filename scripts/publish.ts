...
async function publish(options?: Options): Promise<void> {
  ...
  if (packageNameToVersionOrReleaseType !== undefined) {
    ...
    // 2. Then:
    //    - If found, increment it and keep the main part of the SemVer string
    //      (i.e., the `<major>.<minor>.<patch>` part) unchanged.
    //    - If not found, initialize it to `1` and increment the patch version
    //      number instead.
