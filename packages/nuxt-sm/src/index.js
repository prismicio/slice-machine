...
async function handleLibraryPath(libPath) {
  ...
  if (!pathExists) {
    return logger.warn(
      `[nuxt-sm] path to library "${pathToSlices}" does not exist. Skipping.`
    );
  }
...
async function install(moduleOptions) {
  ...
  const pathToResolver =
    options.pathToResolver ||
    path.join(this.options.srcDir, app, "sm-resolver.js");
  ...
  const smFile = fs.existsSync(pathToSmFile)
  ...
  if (!libraries) {
    return logger.warn(
      '[nuxt-sm] expects a non-empty "libraries" array. If it was intended, consider removing the plugin from your config'
    );
  }

  if (!Array.isArray(libraries) || !libraries.length) {
    return logger.error(
      '[nuxt-sm] expects a "libraries" option to be a non-empty array'
    );
  }
