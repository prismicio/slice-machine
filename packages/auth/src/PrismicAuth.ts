...
import { decode, APIEndpoints, InternalError } from "@slicemachine/misc";

import { serializeCookies } from "./lib/serializeCookies";
import { parseCookies } from "./lib/parseCookies";

import { createPrismicAuthMiddleware } from "./createPrismicAuthMiddleware";
import {
	AUTH_COOKIE_KEY,
	COOKIE_SEPARATOR,
	PERSISTED_AUTH_STATE_FILE_NAME,
	SESSION_COOKIE_KEY,
	SLICE_MACHINE_USER_AGENT,
} from "./constants";
import { PrismicAuthState, PrismicUserProfile } from "./types";
...
type PrismicAuthConstructorArgs = {
	scopedDirectory?: string;
};

type PrismicAuthLoginArgs = {
	email: string;
	cookies: string[];
};

type PrismicAuthGetLoginSessionInfoReturnType = {
	port: number;
	url: string;
};

type PrismicAuthNodeLoginSessionArgs = {
	port: number;
...
export class PrismicAuth {
	...
	async getLoginSessionInfo(): Promise<PrismicAuthGetLoginSessionInfoReturnType> {
	...
	async nodeLoginSession(args: PrismicAuthNodeLoginSessionArgs): Promise<void> {
	...
	async getSerializedAuthenticationCookies(): Promise<string> {
