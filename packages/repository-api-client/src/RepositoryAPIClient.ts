...
import { createPrismicAuth, PrismicAuth } from "@slicemachine/auth";
import { APIEndpoints, decode } from "@slicemachine/misc";
...
type RepositoryAPIClientConstructorArgs = {
	auth?: PrismicAuth;
};

type ReposiotryAPIClientExistsRepositoryArgs = {
	domain: string;
};

type ReposiotryAPIClientCreateRepositoryArgs = {
	domain: string;
	framework: string; // TODO: Type(?)
};

type ReposiotryAPIClientDeleteRepositoryArgs = {
	domain: string;
	password: string;
};

type ReposiotryAPIClientPushDocumentsArgs = {
	domain: string;
	signature: string;
...
export class RepositoryAPIClient {
	...
	constructor({
		auth = createPrismicAuth(),
	}: RepositoryAPIClientConstructorArgs = {}) {
		this.user = auth;
	}
	...
	async checkExists(
		args: ReposiotryAPIClientExistsRepositoryArgs,
	): Promise<boolean> {
		const url = new URL(
	...
	async create(args: ReposiotryAPIClientCreateRepositoryArgs): Promise<void> {
		const url = new URL(
	...
	async delete(args: ReposiotryAPIClientDeleteRepositoryArgs): Promise<void> {
	...
	async pushDocuments(
		args: ReposiotryAPIClientPushDocumentsArgs,
	): Promise<void> {
	...
	private async _fetch(args: {
		...
		const serializedCookies =
			await this.user.getSerializedAuthenticationCookies();
		...
		return fetch(args.url.toString(), {
			...
			headers: {
				...
				"User-Agent": args.userAgent || RepositoryAPIUserAgent.Default,
				...extraHeaders,
			},
		});
	}
