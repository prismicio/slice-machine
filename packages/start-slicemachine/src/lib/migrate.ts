...
export const migrate = async (manager: SliceMachineManager) => {
	try {
		...
		safeUnlink(path.join(createPathToDeprecatedLibrary(manager.cwd), 'assets'), "folder");
		safeUnlink(path.join(createPathToDeprecatedLibrary(manager.cwd), MOCK_CONFIG_FILE_NAME), "file");
	}
}
