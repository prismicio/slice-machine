...
const Chromatic = ({ env }) =>
  env.chromatic
    ? [
        <Button
          key="chromatic-link-1"
          as="a"
          href={env.chromatic.storybook}
          target="_blank"
          ml={2}
          mr={1}
        >
        ...
        <Button
          key="chromatic-link-2"
          as="a"
          href={env.chromatic.library}
          target="_blank"
          mr={1}
        >
          Chromatic preview
        ...
        .then(async (res) => {
          ...
          Object.entries(json).forEach(([key, val]) => {
            if (val?.message) {
              flag = true;
              console.error(
                `Push failed for slice "${key}". Reason: ${val.message}`
              );
            }
          });
  ...
  return (
    ...
    >
      ...
      {memoedWarnings && memoedWarnings.length ? (
        <WarningsIcon
          theme={theme}
          warnings={memoedWarnings.length}
          onClick={openPanel}
        />
      ) : null}
      <IconButton
