...
const compare = (manifest: Manifest, onlinePackage: Manifest, { cwd }: { cwd: string }) => {
...
export function logVersion(npmCompare: Comparison | undefined) {
  ...
  if (!npmCompare.err) {
    logger.plain(
      boxen(
        dedent(`🍕 Slicemachine ${npmCompare?.currentVersion?.split('-')[0]} started.
...
export default async function comparator({ cwd }: { cwd: string }, log: boolean = false) {
