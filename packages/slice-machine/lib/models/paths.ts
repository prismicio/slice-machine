...
export const SliceTemplateConfig = (cwd: string, customPathToTemplate?: string) => customPathToTemplate ? path.join(cwd, customPathToTemplate) : path.join(PrismicConfig(cwd), 'slice-template')
export const Pkg = (cwd: string) => path.join(cwd, 'package.json')
export const YarnLock = (cwd: string) => path.join(cwd, 'yarn.lock')
export const MocksConfig = (cwd: string) => path.join(cwd, '.slicemachine', 'mock-config.json')