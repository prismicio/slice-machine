...
const Form = () => {
  ...
  const [state, setState] = useState({ ...contentValue, err: null, loading: false })
  ...
  const getOembed = async (url) => {
    ...
    .catch(e => {
      console.error(e)
      setState({ url, oembed: null, err: 'Unable to fetch oembed', loading: false })
    })
  }
          ...
          }
          {
            state.oembed ? (
              <Box>
                ...
                <Text mt={1} as="p">Rendered with <a target="_blank" href="https://www.npmjs.com/package/react-oembed-container">react-oembed-container</a></Text>
              </Box>
            ) : null
          }
        </Box>
