...
const VarationsPopover: React.FunctionComponent<{
  ...
  const [current, setCurrent] = useState<Variation<AsArray>>(defaultValue || variations[0])
  ...
  return (
    <div>
      <Popover
        ...
        positions={["bottom"]}
        padding={-1}
        content={() => (<MenuList defaultValue={current} variations={variations} onChange={handleChange} />)}
        containerClassName={"variationSelectorContainer"}
      >
        <Button sx={{ fontSize: 12, padding: 2 }} variant="secondary" onClick={() => setIsOpen(!isOpen)}>Switch Variation</Button>
      </Popover>
    </div>
  )
}
