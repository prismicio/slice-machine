...
const codeByWidgetType = (Widgets) => ({
  [Widgets.ContentRelationship?.CUSTOM_NAME]: (fieldText, useKey) =>
    handleLinkCode(fieldText, useKey),
  [Widgets.LinkToMedia?.CUSTOM_NAME]: (fieldText, useKey) =>
    handleLinkCode(fieldText, useKey),
  [Widgets.UID?.TYPE_NAME]: (fieldText) => `<span>{{ ${fieldText} }}</span>`,
  [Widgets.StructuredText?.TYPE_NAME]: (fieldText, useKey) =>
    `<RichText render={${fieldText}} ${useKey ? appendKey("rich-text") : ""}/>`,
  [Widgets.Image?.TYPE_NAME]: (fieldText, useKey) =>
    `<img src={${fieldText}.url} alt={${fieldText}.alt} ${
      useKey ? appendKey("img") : ""
    }/>`,
  [Widgets.Link?.TYPE_NAME]: (fieldText, useKey) =>
    handleLinkCode(fieldText, useKey),
  [Widgets.Select?.TYPE_NAME]: createDefaultField(),
  [Widgets.Boolean?.TYPE_NAME]: (fieldText, useKey) =>
    `<span ${
  ...
  [Widgets.Color?.TYPE_NAME]: (fieldText, useKey) =>
    `<span  ${
