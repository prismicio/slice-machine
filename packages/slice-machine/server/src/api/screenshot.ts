...
export default async function handler({ from, sliceName }: { from: string, sliceName: string })
  : Promise<{ err: Error | undefined, reason: string | undefined, previews: ReadonlyArray<Preview> }>
{
  ...
  return {
    err,
    reason,
    previews: mergedPreviews.filter(p => p.hasPreview) as ReadonlyArray<Preview>
  }
}