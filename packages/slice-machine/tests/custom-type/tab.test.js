...
test('it resets custom type', () => {
  ...
  expect(result.current[0].current.tabs.length).not.toEqual(result.current[0].initialCustomType.tabs.length)

  act(() => { result.current[1].reset() })

  expect(result.current[0].current.tabs.length).toEqual(result.current[0].initialCustomType.tabs.length)
...
test('it replaces widget', () => {
  ...
  act(() => { result.current[1].tab(key).replaceWidget(widget.key, 'newKey', widget.value) })

  expect(equal(result.current[0].current.tabs[0].value, initialTab.value)).toEqual(false)
  expect(equal(result.current[0].current.tabs[0].value[0].value, initialTab.value[0].value)).toEqual(true)

  act(() => { result.current[1].tab(key).replaceWidget(
  ...
  act(() => {
    result.current[1].tab(key).replaceWidget(
      result.current[0].current.tabs[0].value[0].key,
      result.current[0].current.tabs[0].value[0].key,
      {
        ...widget.value,
  ...
  expect(result.current[0].current.tabs[0].value[0].value.config.placeholder).toEqual(newPlaceholder)
})
...
test('it reorders widgets', () => {
  ...
  act(() => { result.current[1].tab(key).reorderWidget(0, 1) })
  ...
  act(() => { result.current[1].tab(key).reorderWidget(0, 2) })
  ...
  act(() => { result.current[1].tab(key).reorderWidget(2, 0) })
