...
async function main() {
  ...
  console.log('Default screenshots are now stored in a special .slicemachine folder.')
  console.log('To use custom previews, simply keep storing them in your component folder.')
      ...
      try {
        ...
        (json.libraries || []).forEach((lib) => {
          ...
          if (isLocal && pathExists) {
            const matches = glob.sync(`${pathToSlices}/**/preview.png`)
