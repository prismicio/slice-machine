...
test("writes the environment to default the env file", async (ctx) => {
	...
	expect(contents).toMatch(/^NUXT_PUBLIC_PRISMIC_ENVIRONMENT=foo$/m);
});
...
test("updates the variable if the variable exists", async (ctx) => {
	...
	expect(contents).toMatch(/^NUXT_PUBLIC_PRISMIC_ENVIRONMENT=bar$/m);
});
...
test("removes the variable if the environment is undefined", async (ctx) => {
	...
	expect(contents).not.toMatch(/^NUXT_PUBLIC_PRISMIC_ENVIRONMENT=/m);
});
