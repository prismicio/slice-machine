...
import {
	...
	ForbiddenError,
	NotFoundError,
} from "@slicemachine/custom-types-api-client";
import {
	CallHookReturnType,
	HookError,
...
import { DecodeError } from "@slicemachine/misc";
...
export class SlicesManager extends BaseManager {
	...
	async pushSlice(
		...
		if (model) {
			...
			const client = createCustomTypesAPIClient({
			...
			try {
				...
				await client.getSharedSliceByID({ id: args.sliceID });

				// If it exists on the repository, update it.
				await client.updateSharedSlice({ model: modelWithScreenshots });
			} catch (error) {
				if (error instanceof NotFoundError) {
					// If the Slice doesn't exist on the repository, insert it.
					await client.insertSharedSlice({ model: modelWithScreenshots });
