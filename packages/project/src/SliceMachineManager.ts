...
} from "@slicemachine/misc";
import { PrismicAuth, PrismicUserProfile } from "@slicemachine/auth";
...
import { CustomTypesManager } from "./managers/CustomTypesManager";
import { SlicesManager } from "./managers/SlicesManager";
import { SnippetsManager } from "./managers/SnippetsManager";
import { ScreenshotsManager } from "./managers/ScreenshotsManager";
	...
	async getState(): Promise<SliceMachineManagerGetStateReturnType> {
		...
		const versions = await Promise.all(
			allStableVersions.map(async (version): Promise<PackageVersion> => {
				const releaseNotes = await getReleaseNotesForVersion({
					version,
				});
