...
const Slice = ({ slice }) => <h2>{slice.slice_type}</h2>;

const SliceWithProps = ({ customProp }) => <h2>{customProp}</h2>;
...
test("missing prop in slice triggers message", async () => {
  render(<SliceZone slices={[{}]} />);
  expect(screen.getByRole("heading")).toHaveTextContent(
    'Property "slice" not found or not formatted properly'
  );
});

test("renders example slice", async () => {
  render(
    <SliceZone slices={[{ slice_type: "MySlice" }]} resolver={() => Slice} />
  );
...
test("renders slice props", async () => {
  render(
    <SliceZone
      slices={[{ slice_type: "MySlice" }]}
      sliceProps={{ customProp: "My custom prop" }}
      resolver={() => SliceWithProps}
    />
  );
