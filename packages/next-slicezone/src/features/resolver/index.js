...
async function handleLibraryPath(libPath, startPath) {
  const { isLocal, pathExists, pathToSlices } = await getLibraryInfo(
    libPath,
    startPath
  );
...
export const createResolver = async (maybePathToSmFile) => {
  const pathToSmFile =
    maybePathToSmFile || path.posix.join(process.cwd(), SM_FILE);
  ...
  const librariesInfo = await Promise.all(
    libraries.map(async (lib) => await handleLibraryPath(lib, startPath))
  );
...
${body}
  ...
  return fs.writeFileSync(
    path.join(startPath, "sm-resolver.js"),
    file,
    "utf-8"
  );
};
