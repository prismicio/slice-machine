  ...
  if (doc) {
    if (slicesKey) {
      ...
      if (slicesKey in doc.data && Array.isArray(doc.data[slicesKey])) {
        ...
        console.error(
          "[SliceZone/useGetStaticProps] Cannot find slice zone at specified key `%s`\n\nCheck the document below to make sure you provided the right key:",
          slicesKey,
          doc.data
        );
      }
...
export const useGetStaticProps = ({
  ...
  if (uid) {
    console.warn(
      `[next-slicezone/useGetStaticProps]: Parameter "uid" is deprecated, use "apiParams.uid" instead.`
    );
  }
  if (lang) {
    console.warn(
      `[next-slicezone/useGetStaticProps]: Parameter "lang" is deprecated, use "apiParams.lang" instead.`
    );
  }
  if (body) {
    console.warn(
      `[next-slicezone/useGetStaticProps]: Parameter "body" is deprecated, use "slicesKey" instead.`
    );
  }
  ...
  return async function getStaticProps({
    ...
    const finalApiParams =
      apiParams && typeof apiParams === "function"
