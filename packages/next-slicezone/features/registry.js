...
function getFileInfoFromPath(slicePath, componentName) {
  ...
  throw new Error(
    `[nuxt-sm] Could not find module file for component "${componentName}" at path "${slicePath}"`
  );
}

function getComponentInfo(slicePath) {
  const name = getComponentName(slicePath);
  const { fileName, extension, isDirectory } = getFileInfoFromPath(
    slicePath,
    name
  );
  return {
...
async function handleLibraryPath(libPath) {
  ...
  const pathsToComponents = fs
    .readdirSync(slash(pathToSlices))
...
export async function registry(writeFile = false) {
  ...
  if (!libraries) {
    return console.warn(
      '[next-slicezone] expects a non-empty "libraries" array. If it was intended, consider removing the plugin from your config'
    );
  }
