...
describe("add TypeScript types to jsconfig.json or tsconfig.json", () => {
	describe("JavaScript", () => {
		it("adds prismicio-types.d.ts to jsconfig.json", async (ctx) => {
			const filePath = path.join(ctx.project.root, "jsconfig.json");
			...
			expect(contents.includes).toStrictEqual(["./prismicio-types.d.ts"]);
		});

		it("merges with the existing `includes` value", async (ctx) => {
			const filePath = path.join(ctx.project.root, "jsconfig.json");
			...
			expect(contents.includes).toStrictEqual([
				"foo",
				"./prismicio-types.d.ts",
			]);
		});

		it("creates a jsconfig.json file if it doesn't exist", async (ctx) => {
			const preRootDir = await fs.readdir(ctx.project.root);

			expect(preRootDir).not.includes("jsconfig.json");
			...
			const postRootDir = await fs.readdir(ctx.project.root);

			expect(postRootDir).includes("jsconfig.json");
		});
	});

	describe("TypeScript", () => {
		it("adds prismicio-types.d.ts to tsconfig.json", async (ctx) => {
			const filePath = path.join(ctx.project.root, "tsconfig.json");
		...
		it("creates a tsconfig.json file if it doesn't exist", async (ctx) => {
			...
			expect(preRootDir).not.includes("tsconfig.json");
			...
			expect(postRootDir).includes("tsconfig.json");
		});
	});
});
