...
test("follows SvelteKit's env file priority", async (ctx) => {
	await fs.writeFile(
		path.join(ctx.project.root, ".env"),
		"VITE_PRISMIC_ENVIRONMENT=root",
	);
	await fs.writeFile(
		path.join(ctx.project.root, ".env.development"),
		"VITE_PRISMIC_ENVIRONMENT=development",
	);
	await fs.writeFile(
		path.join(ctx.project.root, ".env.local"),
		"VITE_PRISMIC_ENVIRONMENT=local",
	);
	await fs.writeFile(
		path.join(ctx.project.root, ".env.development.local"),
		"VITE_PRISMIC_ENVIRONMENT=development.local",
	);
